function [v_eq,alpha,v_dep,fpa_dep] = gravityAssist(candidateArchitecture,v_arr,fpa_arr,pass_dist)
...
% Departure
v_dep = sqrt(v_planet^2 + v_inf^2 - (2*v_planet*v_inf*cosd(eta+delta))); %departure velocity [km/s]
fpa_dep = asind(v_inf*sind(eta+delta)/v_dep); %departure flight path angle [deg]
